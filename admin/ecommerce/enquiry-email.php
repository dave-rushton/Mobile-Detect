...
if (!is_numeric($Ord_ID)) {
    ...
    $emailBody = $TmpOrd->emailEnquiry($Ord_ID, $mailto, NULL);

    require '../phpmailer/PHPMailerAutoload.php';

    //Commented out as throwing errors - no SMTP Details defined in config (27/11/18 - Shreya)
...
//    $mail->Port = $patchworks->SmtpPort;
    ...
    $mail->Sender = $ecoProp->emaadr;
    $mail->setFrom($ecoProp->emaadr, $patchworks->customerName);
    $mail->addAddress($mailto, $mailto);
    $mail->addReplyTo($patchworks->adminEmail, $patchworks->customerName);
    $mail->isHTML(true);

    $mail->Subject = 'Email Enquiry - Decotel Hospitality';
    $mail->Body = $emailBody;
    $mail->send();
