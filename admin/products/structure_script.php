...
if (isset($_REQUEST['action']) && $_REQUEST['action'] == 'select') {
    ...
    if (!$StrObj) {
        ...
        if (isset($_REQUEST['strimg'])) $StrObj->strimg = $_REQUEST['strimg'];
        ...
        if (isset($_REQUEST['keywrd'])) $StrObj->keywrd = $_REQUEST['keywrd'];
        if (isset($_REQUEST['keydsc'])) $StrObj->keydsc = $_REQUEST['keydsc'];
    ...
    $TmpRel = new RelDAO();
    $related = $TmpRel->structureProducts(NULL, 'PRODUCT', NULL, 'STRUCTURE', $Str_ID, false, false, NULL, NULL, NULL, NULL);
