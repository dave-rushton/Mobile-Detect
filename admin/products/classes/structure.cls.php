...
class StrDAO extends db {
	...
	function select($Str_ID = NULL, $TblNam=NULL, $Tbl_ID = NULL, $ReqObj = false, $strnam = NULL) {
		...
		if (!is_null($Str_ID)) {
			...
			if (!is_null($strnam)) {
				$sql .= ' AND strnam = :strnam ';
				$qryArray["strnam"] = $strnam;
			}
    ...
    function selectBySeo($StrSeo = NULL, $TblNam=NULL, $Tbl_ID = NULL, $ReqObj = false) {
    ...
    function selectLevel($Par_ID = NULL, $TblNam=NULL, $Tbl_ID = NULL, $ReqObj = false, $Sta_ID = NULL) {
        ...
        if (!is_null($Sta_ID)) {
            ...
            else if (is_array($Sta_ID)) {
                $sql .= ' AND sta_id IN (';
                for($i=0;$i<count($Sta_ID);$i++) {
                    $sql .= $Sta_ID[$i];
                    if ($i != (count($Sta_ID) - 1)) {
                        $sql .= ',';
                    }
	...
	function update($StrCls = NULL) {
		...
		if ($StrCls->str_id == 0) {
            ...
            $qryArray["strimg"] = $StrCls->strimg;
            $qryArray["srtord"] = $StrCls->srtord;
			$qryArray["sta_id"] = $StrCls->sta_id;
			$qryArray["keywrd"] = $StrCls->keywrd;
			$qryArray["keydsc"] = $StrCls->keydsc;
...
//        var_dump($qryArray);
    ...
    function buildStructure ($Par_ID = 0, $SeoUrl = NULL, $Ele_ID = NULL, $EleCls = NULL, $admin = false) {
        ...
        for ($i=0;$i<count($structureRecs);$i++) {
            ...
            if ($admin) {
                echo '<a href="' . $structureRecs[$i]['str_id'] . '" class="selectStructureBtn" data-str_id="' . $structureRecs[$i]['str_id'] . '">' . $structureRecs[$i]['strnam'] . '</a>';
            } else {
                echo '<a href="' . $SeoUrl . '/category/' . $structureRecs[$i]['str_id'] . '/' . $structureRecs[$i]['seourl'] . '">';

                echo '<span>Shop For</span>';

                echo $structureRecs[$i]['strnam'] . ' <i></i>';
    ...
    function buildStructureOption ($Par_ID = 0, $SeoUrl = NULL, $Ele_ID = NULL, $EleCls = NULL, $admin = false) {
        ...
        for ($i=0;$i<count($structureRecs);$i++) {
            ...
            $string .= '<option value="'.$structureRecs[$i]['str_id'].'">';
            $string.= $structureRecs[$i]['strnam'];
        ...
        if (count($structureRecs) > 0) {
            ...
            for ($i=0;$i<count($structureRecs);$i++) {
                ...
                if ($admin) {
                    ...
                    echo '<a href="' . $SeoUrl . '/category/' . $structureRecs[$i]['str_id'] . '/' . $structureRecs[$i]['seourl'] . '">' . $structureRecs[$i]['strnam'] . '</a>';
                }
    ...
    function buildStructure2 ($Par_ID = 0, $SeoUrl = NULL, $Ele_ID = NULL, $EleCls = NULL, $admin = false) {
        ...
        if (count($structureRecs) > 0) {
            ...
            for ($i = 0; $i < count($structureRecs); $i++) {
                ...
                $this->ShowSubCats2($structureRecs[$i]['str_id'], $SeoUrl, $admin);
    ...
    function ShowSubCats2($Par_ID, $SeoUrl = NULL, $admin = false)
    {
    ...
    function getBreadcrumb( $Str_ID=NULL, $SeoUrl = '', $Prt_ID = NULL, $language = NULL ) {
        ...
        if ($parentPage) {
            ...
            $breadCrumbHTML = '<li><a href="'.$this->webRoot.$SeoUrl.'">Our Products</a></li></li>'.$breadCrumbHTML;
