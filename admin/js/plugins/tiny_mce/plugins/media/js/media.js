	...
	window.Media = {
		...
		moveStates : function(to_form, field) {
			...
			function setOptions(type, names) {
				...
				if (type == data.type || type == 'cms-form-template.php') {
					names = tinymce.explode(names);
					for (i = 0; i < names.length; i++) {
						name = names[i];
						formItemName = type == 'cms-form-template.php' ? name : type + '_' + name;

						if (type == 'cms-form-template.php')
							list = data;
			...
			setOptions('cms-form-template.php', 'id,name,vspace,hspace,bgcolor,align,width,height');
