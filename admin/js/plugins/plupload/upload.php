...
require_once($patchworks->docRoot."admin/gallery/classes/crop.cls.php");
...
$targetDir = $patchworks->docRoot.'uploads/images/';
            ...
            if ($in) {
                ...
                for ($r = 0; $r < count($resize); $r++) {
                    ...
                    if (count($resizeSizes) > 1) {
                        ...
                        $avatarSize = round($trimWidth,0,PHP_ROUND_HALF_UP).'-'.round($trimHeight,0,PHP_ROUND_HALF_UP);
                        $acWidth = round(0,0,PHP_ROUND_HALF_UP);
                        $acHeight = round(0,0,PHP_ROUND_HALF_UP);
...
//                        if($width > $height){
//                            //LANDSCAPE
//                            $aspect_width = ($height / $trimHeight) * $trimWidth;
//                            $aspect_height = $height;
//                            $center_y = 0;
//                            $center_x = ($width - $aspect_width) / 2;
//                            $center_x = ($width - $aspect_width) / 2;
//                        }
//                        else if($height > $width){
//                            //LANDSCAPE
//                            $aspect_width = $width;
//                            $aspect_height = ($width / $trimWidth) * $trimHeight;
//                            $center_y = ($height - $aspect_height) / 2;
//                            $center_x = 0;
//                        }
//                        else{
...
//                        }
                        ...
                        $position_data = '{"x":' . $center_x . ',"y":' . $center_y . ',"height":' . $aspect_height . ',"width":' . $aspect_width . ',"rotate":0}';


                        $crop = new CropAvatar($patchworks->docRoot."uploads/images/".$fileName, $position_data, NULL, $patchworks->docRoot, $trimWidth.'-'.$trimHeight, $width, $height);
//                        $crop = new CropAvatar($patchworks->docRoot."/uploads/imges/p1ffv5bkmu114rp4g1ev8vkt1uk2c.jpg", NULL, NULL, $patchworks->docRoot, $avatarSize, $acWidth, $acHeight);
