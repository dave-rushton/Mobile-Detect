...
//		echo curl_error($curl_handle) . '<br/>';
    ...
    function getMenu($SeoUrl = NULL, $UseBrk = 0, $ParSeo = NULL, $Men_ID = NULL, $MenCls = NULL, $parent_id = NULL, $showhome = true)
    {
        ...
        if (!empty($parent_id)) {
            ...
            // GET ALL PAGES
            //
            $sql = "SELECT * FROM pages WHERE parent_id = $parent_id  ORDER BY `left`";
            ...
            for ($p = 0; $p < $NumPag; $p++) {
                ...
                if ($pags[$p]['lnktyp'] != 200) {
                    ...
                    if ($pags[$p]['lnktyp'] == 98) {
                        ...
                        $menuStr .= $this->buildStructure(3, $pags[$p]['seourl'], NULL, 'megamenu', false);
                    ...
                    $replaceStr = $this->buildStructure(0, $pags[$p]['seourl'], NULL, 'megamenu', false, [0, 2, 3, 4]);
                    $replaceStr = substr($replaceStr, strpos($replaceStr, '<li>'));
                    $replaceStr = substr($replaceStr, 0, strlen($replaceStr) - 5);
...
//                    $locations = $this->run($sql, $qryArray, false);
    ...
    function buildStructure($Par_ID = 0, $SeoUrl = NULL, $Ele_ID = NULL, $EleCls = NULL, $admin = false, $Sta_ID = NULL)
    {
        ...
        for ($i = 0; $i < count($structureRecs); $i++) {
            ...
            else if ($structureRecs[$i]['sta_id'] == 3 || $structureRecs[$i]['sta_id'] == 4) {
                $strStr .= '<a href="#">';
            ...
            if ($structureRecs[$i]['sta_id'] != 2 && $structureRecs[$i]['sta_id'] != 3) {
                $strStr .= $this->ShowSubCats($structureRecs[$i]['str_id'], $SeoUrl, $admin, 0);
            }
    ...
    function ShowSubCats($Par_ID, $SeoUrl = NULL, $admin = false, $Sta_ID = NULL)
    {
