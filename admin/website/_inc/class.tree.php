...
class _tree_struct {
	...
	// Constructor
	function __construct($table = "pages", $fields = array()) {
		$this->table = $table;
		if(!count($fields)) {
...
class json_tree extends _tree_struct {
	function __construct($table = "pages", $fields = array(), $add_fields = array("title" => "title", "type" => "type")) {
	...
	function search($data) {
		$this->db->query("SELECT `".$this->fields["left"]."`, `".$this->fields["right"]."` FROM `".$this->table."` WHERE LOWER(`".$this->fields["title"]."`) LIKE '%".strtolower($this->db->escape($data["search_str"]))."%'");
