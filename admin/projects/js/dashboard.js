...
function getAvailability() {
	...
	//alert( "projects/availability_dates.php?begdat=" + $('[name="begdat"]', bookingDashForm).val() + "&enddat=" + $('[name="enddat"]', bookingDashForm).val() );
	
	getDependancy($('[name="begdat"]', bookingDashForm).val(), $('[name="enddat"]', bookingDashForm).val())
	
	$.ajax({
		url: "projects/availability_dates.php",
		type: "GET",
		...
		success: function(data) {
			...
			for (var i = 0; i < arrayLength; i++ ) {
				...
				var avlPer = 100 - ((jsonMoney[i] / 8.5) * 100).toFixed(2);
...
function getAnalytics() {
	...
	$.ajax({
		url: "projects/workflow_dates.php",
		type: "GET",
		async: true,
		success: function(data) {
			...
			try {
				...
				// earning warning
				marking.push({ color: "#FF0000", lineWidth: 1, yaxis: { from: 75, to: 75} });
				...
				$("#lineChart").bind("plotclick", function (event, pos, item) {
					...
					window.location = 'projects/calendar.php?y='+caldata[0]+'&m='+caldata[1]+'&d='+caldata[2];
