...
if (!isset($shoppingcart->shoppingCart['orderToken']) || !is_numeric($shoppingcart->shoppingCart['orderToken'])) {
    ...
    $Ord_ID = $shoppingcart->convertSessionToOrder(NULL, $loggedIn);
    $orderToken = $Ord_ID;

} else {

    $Ord_ID = $shoppingcart->convertSessionToOrder( $shoppingcart->shoppingCart['orderToken'], $loggedIn );
...
//$strEncryptionPassword="f2EgKg0iB7rkM2D7";    /** Set this value to the XOR Encryption password assigned to you by Sage Pay **/
//$strEncryptionPassword="Er57aXh8aPcvYXoS";
$strEncryptionPassword= $ecoProp->sp_enc; //"d9df74fe9f225374";
...
if (isset($shoppingcart->shoppingCart['items']) && is_array($shoppingcart->shoppingCart['items'])) {
	...
	for ($i=0;$i<count($shoppingcart->shoppingCart['items']);$i++) {
        ...
        if (isset($productRec->unipri)) {
            ...
            $DisQty = $patchworks->getJSONVariable($productRec->prtobj, 'disqty', false);
            $DisPri = $patchworks->getJSONVariable($productRec->prtobj, 'dispri', false);

            $UniPri = $productRec->unipri;

            //echo $productRec->prd_id.' '.$UniPri.':';

            if ($DisQty > 0 && $shoppingcart->shoppingCart['items'][$i]['qty'] >= $DisQty && $DisPri < $UniPri) $UniPri = $DisPri;

        }

        $multiPrice = $shoppingcart->multiBuyPrice($shoppingcart->shoppingCart['items'][$i]['prt_id']);

        if (is_numeric($multiPrice) && $multiPrice > 0) $UniPri = $multiPrice;

        if ($productRec->buypri > $UniPri) $UniPri = $productRec->buypri;

        $relatedIDs .= (empty($relatedIDs)) ? $shoppingcart->shoppingCart['items'][$i]['prd_id'] : ',' . $shoppingcart->shoppingCart['items'][$i]['prd_id'];
...
if (!$TmpOrd->inEurope($shoppingcart->shoppingCart['customer']['paycoucod'])) {
...
$full = $shoppingcart->shoppingCart['customer']['payfao'];
$full1 = explode(' ', $full);
$first = $full1[0];
...
$full = $shoppingcart->shoppingCart['customer']['fao'];

if (empty($full)) $full = $shoppingcart->shoppingCart['customer']['payfao'];
...
<?php
//echo '<p style="font-size: 2px;">'.$strPost.'</p>';
?>