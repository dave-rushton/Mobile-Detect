...
class shoppingCart extends db
{
    ...
    function priceProduct($Prd_ID = NULL, $NumUni = NULL, $Cus_ID) {
            ...
            if ($product) {
                ...
                if (isset($this->shoppingCart['items']) && is_array($this->shoppingCart['items'])) {
                    for ($i = 0; $i < count($this->shoppingCart['items']); $i++) {
                        ...
                        if ( $this->shoppingCart['items'][$i]['prd_id'] != $Prd_ID ) continue;

                        if ( !in_array($this->shoppingCart['items'][$i]['prt_id'], $prtArray)) {

                            array_push($prtArray, $this->shoppingCart['items'][$i]['prt_id']);
                            array_push($priArray, $this->shoppingCart['items'][$i]['unipri']);
                ...
                for ( $p = 0; $p < count($prtArray); $p++ ) {
                    ...
                    $qryArray = array();
                    $sql = 'SELECT * FROM pricebands WHERE prt_id = :prt_id AND numuni <= :numuni AND (IS NULL prd_id OR prd_id = "") ORDER BY unipri ASC';
...
//                }
//
//                // find price band for product qty
//
//                if ($deBug) echo 'after prt check<br>';
//                if ($deBug) echo $NumUni.' '.$UniPri.'<br>';
        ...
        if (isset($this->shoppingCart['items']) && is_array($this->shoppingCart['items'])) {
            for ($i = 0; $i < count($this->shoppingCart['items']); $i++) {
                if ($this->shoppingCart['items'][$i]['prd_id'] == $Prd_ID) {
                    ...
                    //if ( $this->shoppingCart['items'][$i]['qty'] > $product->in_stk ) $this->shoppingCart['items'][$i]['qty'] = $product->in_stk;
    ...
    function convertSessionToOrder($Ord_ID = 0, $loggedIn = NULL, $PayTyp = '')
    {
        ...
        if (is_array($this->shoppingCart['items'])) {
            ...
            $OrdObj->del_id = $this->shoppingCart['delivery']['del_id'];
            ...
            //
            if (is_array($this->shoppingCart['delivery'])) {
